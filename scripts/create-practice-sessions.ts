...
async function createAllPracticeSessions() {
  try {
    ...
    console.log('\nAll practice sessions have been created successfully!');
    console.log('Summary:');
    const totalSessions = await db.select({ count: { count: () => 'count(*)' } }).from(practiceSessions);
    console.log(`Total practice sessions in database: ${totalSessions[0].count}`);
...
export { createAllPracticeSessions };